...
On **Ads**, paste a Meta user token (`ads_management`, `ads_read`), ad account ID, Pixel ID, and Facebook Page ID. Save installs the Pixel on the Horizon theme. **Create paused campaign** builds a traffic campaign to https://fernora.nz for New Zealand and Australia. **Go live** spends only the daily cap. Paid Shopify orders send Purchase events to the Pixel from Pressroom.
